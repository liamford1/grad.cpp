...
MetricsLog::MetricsLog(const std::string& path, bool append, int total_steps, long tokens_per_step,
    ...
    out_ << "m," << total_steps << "," << tokens_per_step << "," << param_count << "," << model_desc
         << "\n";
    maybe_flush(true);
}
...
void MetricsLog::log_step(int step, float loss, float lr, float grad_norm, long step_ms,
                          long mem_mb) {
    out_ << "t," << step << "," << loss << "," << lr << "," << grad_norm << "," << step_ms << ","
         << mem_mb << "," << fixed_wall(wall_s()) << "\n";
    maybe_flush();
}
...
void TrainingMetrics::record_step(int step, float loss, float grad_norm) {
    ...
    if (step % 100 == 0) {
        ...
        std::cout << std::setw(10) << step << std::setw(15) << std::fixed << std::setprecision(6)
                  << loss << std::setw(15) << std::fixed << std::setprecision(4) << grad_norm
                  << "  (" << elapsed << "s)"
                  << " [" << get_memory_mb() << "MB]" << std::endl;
    }
}
...
void TrainingMetrics::print_progress(int step, float loss) {
    ...
    float steps_per_sec =
        static_cast<float>(step + 1 - start_step_) / static_cast<float>(elapsed + 1);
    int eta_sec =
        static_cast<int>(static_cast<float>(total_steps_ - step - 1) / (steps_per_sec + 0.0001f));

    std::cout << "\r[" << (step + 1) << "/" << total_steps_ << "] " << std::fixed
...
void TrainingMetrics::print_summary() {
    ...
    if (seconds > 0.0 && step_count_ > 0) {
        std::cout << "Speed: " << std::fixed << std::setprecision(2) << step_count_ / seconds
                  << " steps/s" << std::endl;
    }

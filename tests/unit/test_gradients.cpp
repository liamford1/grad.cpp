...
void test_simple_addition() {
    ...
    std::cout << (passed ? "✓ Addition gradient correct" : "✗ Addition gradient FAILED")
              << std::endl;
}
...
void test_matmul() {
    ...
    std::cout << "Expected: (1+" << epsilon << ")*0.5 + 2*1.5 + 3*2.5 = "
              << ((original + epsilon) * 0.5f + 2.0f * 1.5f + 3.0f * 2.5f) << std::endl;

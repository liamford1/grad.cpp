...
float model_loss(const GPTModel& model, const std::shared_ptr<Variable>& input,
...
float numerical_gradient(const GPTModel& model, const std::shared_ptr<Variable>& input,
                         const std::shared_ptr<Variable>& targets, float* param,
                         float epsilon = 1e-2f) {
...
bool check_point(const std::string& name, float analytical, float numerical, int& passed,
                 int& total) {
    float abs_err = std::abs(analytical - numerical);
    float rel_err = abs_err / (std::abs(analytical) + std::abs(numerical) + 1e-8f);
    ...
    std::cout << (ok ? "  PASS " : "  FAIL ") << name << ": analytical=" << analytical
              << " numerical=" << numerical << " rel_err=" << rel_err << std::endl;
    total++;
    if (ok) passed++;
    return ok;
}
...
void run_arch(GPTArch arch, const std::vector<Probe>& probes, int& passed, int& total) {
    ...
    for (const auto& probe : probes) {
        int idx = probe.param_idx >= 0 ? probe.param_idx
        ...
        float numerical =
            numerical_gradient(model, input, targets, &p->getData().raw()[probe.flat_idx]);
...
int main() {
    ...
    std::cout << "\n=== FULL-MODEL GRADIENT CHECK (Modern arch: RMSNorm+RoPE+SwiGLU) ==="
              << std::endl;

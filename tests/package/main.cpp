...
int main() {
    BPETokenizer tokenizer(40);
    tokenizer.train("the cat sat on the mat");
    const std::vector<int> ids = tokenizer.encode("the cat");

    GPTModel model(tokenizer.getCurrentVocabSize(), 16, 1, 2, 8, 0.0f);
    Tensor input(static_cast<int>(ids.size()), 1);
    for (size_t i = 0; i < ids.size(); i++) {
        input.setValue(static_cast<int>(i), 0, static_cast<float>(ids[i]));
    }
    auto logits = model.forward(Variable::create(input, false), false);
